                                      ...
                                      const sensor_msgs::CameraInfoConstPtr& info_msg)
{
  ...
  if ( rgb_msg->width == depth_msg->width )
  {
  ...
  } else if ( rgb_msg->width > depth_msg->width )
  {
    for (int v = 0; v < (int)cloud_msg->height; ++v)
    {
      int depth_offset = (v >> shift) * depth_msg->width;
      for (int u = 0; u < (int)cloud_msg->width; ++u, color_idx += color_step, ++pt_iter)
      {
        pcl::PointXYZRGB& pt = *pt_iter;

				depth_idx = depth_offset + (u >> shift);
